			...
			<table class="table table-striped table-bordered table-hover">
				<tbody>
					<tr>
						...
						<td><input class="form-control w400" value="{DATA.title}" name="title" id="title" maxlength="100" /><input
					...
					<tr>
						...
						<td><input class="form-control w200 pull-left" value="{DATA.from_time}" type="text" id="from_time"
								name="from_time" readonly="readonly" /></td>
					</tr>
					...
					<tr>
						...
						<td><input class="form-control w400" value="{DATA.from_org}" name="from_org" id="from_org"
								maxlength="100" /></td>
					</tr>
					<tr>
						<td>{LANG.dis_to_org}</td>
						<td><textarea class="form-control w400" name="to_org">{DATA.to_org}</textarea><span
								class="help-block">{LANG.org}</span></td>
					</tr>
					...
					<tr>
						...
						<td>
							<select class="form-control w200" name="from_signer" id="from_signer_{LISSIS.id}"
								onchange="nv_link('{LISSIS.id}');">
								<!-- BEGIN: from_signer -->
								<option value="{LISSIS.id}" {LISSIS.selected}>{LISSIS.name}</option>
								<!-- END: from_signer -->
							</select><span id="hienthi"> {position} </span>
						</td>
					</tr>

					<tr>
						<td>{LANG.dis_date_iss}(<span class="red">*</span>)</strong></td>
						<td><input class="form-control w200 pull-left" value="{DATA.date_iss}" type="text" id="date_iss"
								name="date_iss" readonly="readonly" /></td>
					</tr>
					<tr>
						<td>{LANG.dis_date_first}(<span class="red">*</span>)</strong></td>
						<td><input class="form-control w200 pull-left" value="{DATA.date_first}" type="text" id="date_first"
								name="date_first" readonly="readonly" /></td>
					</tr>

					<tr>
						<td>{LANG.dis_date_die}</td>
						<td><input class="form-control w200 pull-left" value="{DATA.date_die}" type="text" id="date_die"
								name="date_die" readonly="readonly" /></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							<div id="fileupload_items">
								<!-- BEGIN: fileupload -->
								<label>
									<input class="form-control w400 pull-left" style="margin-right: 5px" type="text"
										value="{FILEUPLOAD.value}" name="fileupload[]" id="fileupload{FILEUPLOAD.key}" style="width: 300px"
										maxlength="255" />
								</label>
								<!-- END: fileupload -->
							</div>
					...
					<tr>
						...
						<td>
							<div
								style="padding: 4px; height: 150px; width: 400px; background: none repeat scroll 0% 0% rgb(255, 255, 255); overflow: auto; text-align: left; border: 1px solid rgb(204, 204, 204);">
								<table>
									<tr>
										<td>
											<input id="check_all[]" type="checkbox" value="yes"
												onclick="nv_checkAll(this.form, 'idcheck[]', 'check_all[]',this.checked);" />{LANG.dis_cho}
										</td>
									</tr>
									<!-- BEGIN: loop -->
									<tr>
										<td>
											<input name="deid[{ROW.id}]" value="{ROW.id}" type="checkbox" {ROW.checked} id="idcheck[]" />
											{ROW.name}
										</td>
									</tr>
									<!-- END: loop -->
					...
					<tr>
						...
						<td>
							<!-- BEGIN: groups_view -->
							<div class="row">
								<label><input name="groups_view[]" type="checkbox" value="{groups_view.value}"
										{groups_view.checked} />{groups_view.title}</label>
							</div>
							<!-- END: groups_view -->
						</td>
					</tr>
					<tr>
						<td colspan="2"><input class="btn btn-primary" type="submit" name="submit" value="Lưu" /></td>
					</tr>
				</tbody>
			</table>
		</form>
