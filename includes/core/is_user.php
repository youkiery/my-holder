...
if (defined('NV_IS_ADMIN')) {
    ...
    // pcw: đăng nhập bằng session tự cấu hình
    if (!empty($session = $_SESSION['session'])) {
        $sql = "select * from pet_users_session where session = '$session'";
        $userSession = $db->fetch($sql);

        if (!empty($userSession)) {
            if ($userSession['time'] > time()) {
                ...
                FROM ' . NV_USERS_GLOBALTABLE . ' WHERE userid = ' . $userSession['userid'] . ' AND active=1';
                ...
                $sql = "update pet_users_session set time = $time where session = '$session'";
            }
            else {
                $sql = "delete from pet_users_session where session = '$session'";
            }
            $db->query($sql);
        }

    }
    else if ($nv_Request->get_bool('nvloginhash', 'cookie', false)) {
